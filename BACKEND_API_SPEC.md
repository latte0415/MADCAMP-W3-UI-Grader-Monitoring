...
```json
{
  ...
  "processing_tasks": [
    {
      "worker_id": "string",
      "worker_name": "string (alternative field name)",
      "type": "string (task type)",
      "action_type": "string (alternative field name)",
      "task_type": "string (alternative field name)",
  ...
  // 또는
  "processing": [...] // processing_tasks 대신 processing 필드명도 지원
}
```

#### 중요 사항

- **processing_tasks**: 현재 처리 중인 모든 작업의 목록입니다
  - 각 작업 객체는 최소한 `worker_id` 또는 `worker_name` 필드를 포함해야 합니다
  - `type`, `action_type`, 또는 `task_type` 중 하나는 포함되어야 합니다
  - `run_id`가 있으면 해당 Run과 연결된 작업임을 표시합니다
  - 전체 작업 객체는 JSON으로 표시되므로 모든 관련 필드를 포함하는 것이 좋습니다
...
- **workers**: 특정 run_id와 관련된 워커 목록입니다
  - 각 워커는 `worker_id` 또는 `worker_name` 필드를 포함해야 합니다
  - `current_task` 필드는 해당 워커가 현재 처리 중인 작업의 상세 정보를 포함합니다
  - `current_task`가 없으면 해당 워커는 현재 작업을 처리하고 있지 않음을 의미합니다
